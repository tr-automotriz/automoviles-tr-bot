...
async function generarPDF(tipo, datosDoc, datosVeh) {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(22).font('Helvetica-Bold').fillColor('#ffffff')
       .text('AUTOMÓVILES TR', 50, 55, { align: 'center', width: 495 });
    doc.fontSize(10).font('Helvetica').fillColor('#aaaacc')
       .text('Retamal & Toledo  |  Puerto Montt, Chile  |  +56 9 5702 4102', 50, 82, { align: 'center', width: 495 });
    doc.moveDown(2);

    const titulo = tipo === 'compra' ? 'NOTA DE COMPRA' : 'NOTA DE VENTA';
    doc.fontSize(15).font('Helvetica-Bold').fillColor('#1a1a2e').text(titulo, { align: 'center' });
    doc.fontSize(11).font('Helvetica').fillColor('#555')
       .text(`Folio N°: ${datosDoc.numDoc}     Fecha: ${fecha}`, { align: 'center' });
    doc.moveDown();
    doc.moveTo(50, doc.y).lineTo(545, doc.y).lineWidth(1.5).strokeColor('#1a1a2e').stroke();
    doc.moveDown(0.5);

    const fila = (label, valor) => {
      if (valor !== null && valor !== undefined && valor !== '') {
        doc.font('Helvetica-Bold').text(label + ' ', { continued: true }).font('Helvetica').text(String(valor));
      }
    };

    // Datos del vehículo
    doc.fontSize(12).font('Helvetica-Bold').fillColor('#1a1a2e').text('DATOS DEL VEHÍCULO');
    doc.moveDown(0.3);
    doc.fontSize(11).fillColor('#333');
    fila('Vehículo:', `${datosVeh.marca} ${datosVeh.modelo} ${datosVeh.anio}`);
    fila('Patente:', datosVeh.patente);
    fila('Kilometraje:', datosVeh.km ? `${Number(datosVeh.km).toLocaleString('es-CL')} km` : null);
    fila('Color:', datosVeh.color);
    fila('Combustible:', datosVeh.combustible);
    doc.moveDown(0.8);
    ...
    // Datos persona
    if (tipo === 'compra') {
      doc.fontSize(12).font('Helvetica-Bold').fillColor('#1a1a2e').text('DATOS DEL VENDEDOR');
      doc.moveDown(0.3);
      doc.fontSize(11).fillColor('#333');
      fila('Nombre:', datosDoc.vendedor_nombre);
      fila('RUT:', datosDoc.vendedor_rut);
    } else {
      doc.fontSize(12).font('Helvetica-Bold').fillColor('#1a1a2e').text('DATOS DEL COMPRADOR');
      doc.moveDown(0.3);
      doc.fontSize(11).fillColor('#333');
      fila('Nombre:', datosDoc.comprador_nombre);
      fila('RUT:', datosDoc.comprador_rut);
    }
    doc.moveDown(0.8);
    doc.moveTo(50, doc.y).lineTo(545, doc.y).lineWidth(0.5).strokeColor('#cccccc').stroke();
    doc.moveDown(0.5);
    ...
    // Detalle económico
    doc.fontSize(12).font('Helvetica-Bold').fillColor('#1a1a2e').text('DETALLE ECONÓMICO');
    doc.moveDown(0.3);
    doc.fontSize(11).fillColor('#333');
    if (tipo === 'compra') {
      fila('Monto de compra:', `$${Number(datosDoc.monto).toLocaleString('es-CL')}`);
      fila('Forma de pago:', datosDoc.forma_pago || 'Transferencia');
      fila('Tipo de título:', datosDoc.tipo_titulo);
    } else {
      fila('Precio de venta:', `$${Number(datosDoc.precio_venta).toLocaleString('es-CL')}`);
      fila('Forma de pago:', datosDoc.forma_pago || 'Transferencia');
      if (datosDoc.ganancia !== undefined) {
        fila('Ganancia neta:', `$${Number(datosDoc.ganancia).toLocaleString('es-CL')}`);
      }
    }
    if (datosDoc.obs) { doc.moveDown(0.3); fila('Observaciones:', datosDoc.obs); }

    // Pie
    doc.moveDown(3);
    doc.moveTo(50, doc.y).lineTo(545, doc.y).lineWidth(1).strokeColor('#1a1a2e').stroke();
    doc.moveDown(0.5);
    doc.fontSize(9).fillColor('#888')
       .text('Documento generado automáticamente por Bot Automóviles TR', { align: 'center' })
       .text('tr-automotriz.github.io/automoviles-tr', { align: 'center' });
    doc.end();
  });
}
...
app.post('/webhook', async (req, res) => {
  ...
  try {
    ...
    if (datos.tipo === 'compra') {
      ...
      const pdfBuffer = await generarPDF('compra', { numDoc, vendedor_nombre: datos.vendedor_nombre, vendedor_rut: datos.vendedor_rut, monto: datos.monto, forma_pago: datos.forma_pago, tipo_titulo: datos.tipo_titulo, obs: datos.obs }, { marca: datos.marca, modelo: datos.modelo, anio: datos.anio, patente: datos.patente.toUpperCase(), km: datos.km, color: datos.color, combustible: datos.combustible });
      await subirYEnviarPDF(to, pdfBuffer, `${numDoc}-${datos.patente.toUpperCase()}.pdf`, `✅ *Compra registrada ${numDoc}*\n🚗 ${datos.marca} ${datos.modelo} ${datos.anio}\n🔑 ${datos.patente.toUpperCase()}\n💰 $${Number(datos.monto).toLocaleString('es-CL')}\n👤 ${datos.vendedor_nombre}`);
      return;
    }
    ...
    if (datos.tipo === 'venta') {
      ...
      const pdfBuffer = await generarPDF('venta', { numDoc, comprador_nombre: datos.vendedor_nombre, comprador_rut: datos.vendedor_rut, precio_venta: datos.monto, forma_pago: datos.forma_pago, ganancia, obs: datos.obs }, { marca: veh.marca, modelo: veh.modelo, anio: veh.anio, patente: veh.patente, km: veh.km, color: veh.color, combustible: veh.combustible });
      await subirYEnviarPDF(to, pdfBuffer, `${numDoc}-${veh.patente}.pdf`, `✅ *Venta registrada ${numDoc}*\n🚗 ${veh.marca} ${veh.modelo} ${veh.anio}\n🔑 ${veh.patente}\n💰 $${Number(datos.monto).toLocaleString('es-CL')}\n📈 Ganancia: $${Number(ganancia).toLocaleString('es-CL')}\n👤 ${datos.vendedor_nombre}`);
      return;
    }

    await sendMsg(to, `🚗 *Automóviles TR - Bot*\n\n📥 *Compra:* "Compré un Toyota Corolla 2018 patente AB1234, 95000 km, vendedor Juan Pérez, precio 6500000"\n\n📤 *Venta:* "Vendí el AB1234 en 8000000 al Sr. Pedro González"\n\n📋 *Stock:* "¿Qué vehículos tenemos?"`);
